...
void DepthBufferArray::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr)
{
	...
	CreateDerivedViews(Graphics::g_Device, Format, DepthOrArraySize);
}

void DepthBufferArray::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, uint32_t Samples, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr)
{
	D3D12_RESOURCE_DESC ResourceDesc = DescribeTex2D(Width, Height, 1, 1, Format, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
...
void DepthBufferArray::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, DXGI_FORMAT Format, EsramAllocator&)
{
	Create(Name, Width, Height, DepthOrArraySize, Format);
}

void DepthBufferArray::Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, uint32_t Samples, DXGI_FORMAT Format, EsramAllocator& Allocator)
{
	Create(Name, Width, Height, Samples, DepthOrArraySize, Format);
}

void DepthBufferArray::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format, uint32_t ArraySize)
{
	...
	else
	{
		...
		SRVDesc.Texture2DMSArray.ArraySize = (UINT)ArraySize;
	}
