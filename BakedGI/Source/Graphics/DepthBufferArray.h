...
class DepthBufferArray : public PixelBuffer
{
public:
	DepthBufferArray(float ClearDepth = 0.0f, uint8_t ClearStencil = 0)
	...
	void Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, DXGI_FORMAT Format,
	...
	void Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, uint32_t NumSamples, DXGI_FORMAT Format,
		D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr = D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN);
	void Create(const std::wstring& Name, uint32_t Width, uint32_t Height, uint32_t DepthOrArraySize, uint32_t NumSamples, DXGI_FORMAT Format,
		EsramAllocator& Allocator);
